    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (profileData) {
          ...
          // Fallback for Local Bridge users
          setProfile({
            full_name: user.user_metadata.full_name || "User",
            business_name: user.user_metadata.business_name || null,
            phone: user.id.includes("@") ? null : user.id,
            email: user.email || null
          });
        }

        // Only fetch cloud bills/orders if we are NOT a local user
        const isLocalUser = user.user_metadata?.isLocal === true;
