...
const ProductsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoadingProducts(true); // Set loading true at the start of fetch
      ...
      try {
        ...
        if (localResp.ok) {
          ...
          const mappedProducts = localData.map((p: { sku?: string; name: string; price: number; category?: string; stock_quantity: number }, index: number) => ({
      ...
      try { // Wrap Supabase fetch in try-catch as well
        ...
        if (data) {
          const mappedProducts = data.map((p) => ({
            ...
            gstRate: (p as { gst_rate?: number }).gst_rate || 18 // fallback
          }));
          setProducts(mappedProducts);
        }
      } catch (error) {
